...
$query2 = "SELECT * FROM tbl_t2s_229 WHERE student_id=". $myId;   //getting detaild of student
  ...
  <div class="collapse navbar-collapse" id="navbarNav">
    <ul class="navbar-nav">
      ...
      <li class="nav-item active">
        <a class="nav-link" href="#">המורים שלי</a>
      </li>
      <li class="nav-item">
  ...
  <main>
    <section class="studentSec">
      ...
      while($row = mysqli_fetch_assoc($result2)) {
        $query = "SELECT * FROM tbl_teacher_229 WHERE id=" . $row['teacher_id'];
        $result = mysqli_query($connection, $query);
        if(!$result) {
          die("DB query failed.");
        }
        $row1 = mysqli_fetch_assoc($result);
        echo "      <a href='myTeacher.php?id=" . $myId . "&teacher_id=" . $row1['id'] . "'><div id='card3' class='card' style='width: 18rem;'>
                <div class='card-body'>
